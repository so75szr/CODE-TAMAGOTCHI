...
const char* aliments[] = {"Pomme"}; // Ajoutez plus d'aliments ici si nécessaire
...
void loop() {
    ...
    if (menuMangerAffiche) {
        if (digitalRead(PIN_BOUTON_MILIEU) == LOW) {
            selectionAliment = (selectionAliment + 1) % 4;  // Assumant qu'il y a 4 aliments
            afficherMenuManger();
            delay(250);
        }
        if (digitalRead(PIN_BOUTON_DROITE) == LOW) {
        ...
        if (digitalRead(PIN_BOUTON_GAUCHE) == LOW) {
            delay(250);
            // Logique pour valider l'option de l'aliment sélectionné (ajoutez votre code ici)
        }
    }
